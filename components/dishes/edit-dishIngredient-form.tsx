...
export default function EditDishIngredientForm({
    dishIngredient,
    isOpen,
    setIsOpen,
}: Props) {
    ...
    async function onSubmit(values: z.infer<typeof recipeSchema>) {
        ...
        try {
            ...
            setIsOpen(false);
            toast({
                description: `${dishIngredient.name} was updated`,
                duration: 3000,
            });
            form.reset();
            router.refresh();
        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            <DialogContent className="w-[250px]">
                <DialogHeader className="mb-5">
                    <DialogTitle>{dishIngredient.name}</DialogTitle>
                </DialogHeader>
                <Form {...form}>
                    <form
