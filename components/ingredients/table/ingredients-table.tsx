...
import { IngredientT, columnsT } from "@/types";
...
export default function DataTable({ data, columns }: Props) {
    ...
    const renderCell = useCallback(
        (ingredient: any, columnKey: React.Key) => {
            const cellValue = ingredient[columnKey as keyof IngredientT];

            switch (columnKey) {
                case "price":
                    return <p>{ingredient.price.toFixed(2)}€</p>;
                case "updatedAt":
                    return <p>{formatDate(ingredient.updatedAt)}</p>;
                case "actions":
                    return (
